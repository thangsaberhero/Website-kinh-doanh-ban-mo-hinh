      ...
      <main class="flex-1 overflow-y-auto p-6 lg:p-12 custom-scrollbar relative">
        ...
        <div class="glass-panel p-5 md:p-10 rounded-2xl relative overflow-hidden border border-outline-variant/20 shadow-2xl">          
          <form @submit.prevent="saveProfile" class="relative z-10 space-y-10">
            ...
            <div class="pt-8 flex flex-row items-center justify-end gap-3 border-t border-outline-variant/15">
              <button type="button" @click="fetchUserData" class="flex-1 sm:flex-none px-4 py-3.5 text-xs font-bold uppercase tracking-widest text-white bg-surface-container-highest border border-outline-variant/30 rounded-lg hover:bg-surface-bright transition-colors text-center">
                Hủy thay đổi
              </button>
              <button type="submit" class="flex-[2] sm:flex-none bg-gradient-to-r from-primary to-primary-container text-on-primary-fixed font-headline font-bold uppercase tracking-widest px-4 sm:px-8 py-3.5 rounded-lg neon-glow hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2">
