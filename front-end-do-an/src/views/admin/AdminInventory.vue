                    ...
                    <td class="px-8 py-4">
                      <div class="flex items-center gap-4">
                        <div class="w-16 h-16 bg-slate-100 rounded-xl border border-slate-200 overflow-hidden shadow-inner shrink-0 p-0.5">
                          <img :src="product.thumbnailUrl || `https://ui-avatars.com/api/?name=${product.name.charAt(0)}&background=random&color=fff&size=128`" class="w-full h-full object-cover rounded-lg"/>
                        </div>
...
<script setup>
  ...
  const submitNewProduct = () => {
      ...
      if(newProduct.value.thumbnailFile) formData.append('AnhDaiDien', newProduct.value.thumbnailFile);
      newProduct.value.galleryFiles.forEach(file => formData.append('BoSuuTapAnh', file));
      ...
      products.value.unshift({
          ...
          thumbnailUrl: newProduct.value.thumbnailUrl
      });
  ...
  // DỮ LIỆU GIẢ LẬP 
  const products = ref([
    { id: 1, name: 'Gundam Exia Repair IV', brand: 'Bandai', category: 'Action Figure', variant: 'Bản Tiêu Chuẩn', scale: 'Metal Build', costPrice: '4.200.000', sellPrice: '5.850.000', stock: 42, galleryUrls: [] },
    { id: 2, name: 'Iron Man Mark LXXXV', brand: 'Hot Toys', category: 'Figure Tỷ Lệ', variant: 'Bản Deluxe', scale: '1/6 Scale', costPrice: '8.500.000', sellPrice: '11.200.000', stock: 3, galleryUrls: [] },
    { id: 3, name: 'Hatsune Miku: Magical Mirai', brand: 'GSC', category: 'Chibi Figure', variant: 'Nendoroid No. 1200', scale: 'Non-scale', costPrice: '1.100.000', sellPrice: '1.650.000', stock: 12, galleryUrls: [] },
    { id: 4, name: 'Super Saiyan Son Goku', brand: 'Bandai', category: 'Statue (Tĩnh)', variant: 'Masterlise', scale: 'Ichiban Kuji', costPrice: '850.000', sellPrice: '1.350.000', stock: 8, galleryUrls: [] },
  ]);
