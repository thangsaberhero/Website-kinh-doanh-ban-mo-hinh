    ...
    <div class="flex-1 flex flex-col h-screen w-full relative">
      ...
      <div class="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex items-center justify-between shadow-sm">
        ...
        <div class="flex items-center gap-3">
          ...
          <button @click="submitPost('Đã duyệt')" class="px-6 py-2.5 rounded-xl font-bold text-white bg-primary hover:bg-[#ff3d00] shadow-lg shadow-primary/20 transition-all active:scale-95 text-sm flex items-center gap-2">
      ...
      <main class="flex-1 overflow-y-auto p-8 custom-scrollbar pb-24">        
        <div class="flex flex-col xl:flex-row gap-8 max-w-7xl mx-auto">          
          <div class="flex-1 flex flex-col gap-6">          
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-1 transition-all focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10">
              <input 
          ...
          <div class="w-full xl:w-80 flex flex-col gap-6 shrink-0">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              <h3 class="font-bold text-slate-900 mb-4 uppercase tracking-widest text-[11px] flex items-center gap-2">
                <span class="material-symbols-outlined text-primary text-[18px]">category</span> Chuyên mục
              </h3>
              ...
              <div class="relative">
                <input 
                  ...
                  class="w-full border border-slate-200 rounded-xl px-4 py-3 pr-10 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium text-slate-700 bg-slate-50"
                >
                ...
                <Transition name="fade-slide">
                  <ul v-if="showCategoryDropdown" class="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-48 overflow-y-auto custom-scrollbar overflow-hidden py-1">
                    <li 
                      ...
                      class="px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-primary/10 hover:text-primary cursor-pointer transition-colors flex items-center justify-between"
                    >
                      {{ cat }}
                      <span v-if="postData.category === cat" class="material-symbols-outlined text-[16px] text-primary">check</span>
                    </li>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6">
              <h3 class="font-bold text-slate-900 mb-4 uppercase tracking-widest text-[11px] flex items-center gap-2">
                <span class="material-symbols-outlined text-primary text-[18px]">image</span> Ảnh đại diện (Thumbnail)
              </h3>
              
              <input type="file" ref="fileInputRef" class="hidden" accept="image/*" @change="handleFileUpload">
              
              <div @click="triggerFileInput" class="border-2 border-dashed rounded-xl h-44 flex flex-col items-center justify-center gap-2 transition-all cursor-pointer group relative overflow-hidden" :class="postData.thumbnail ? 'border-transparent p-0' : 'border-slate-300 hover:border-primary hover:bg-primary/5 bg-slate-50'">
                <template v-if="!postData.thumbnail">
                  <span class="material-symbols-outlined text-4xl text-slate-300 group-hover:text-primary mb-1 transition-colors">add_photo_alternate</span>
                  <span class="text-xs font-bold text-slate-500 group-hover:text-primary">Click để tải ảnh lên</span>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6">
              <h3 class="font-bold text-slate-900 mb-4 uppercase tracking-widest text-[11px] flex items-center gap-2">
                <span class="material-symbols-outlined text-primary text-[18px]">manage_search</span> Tóm tắt bài viết 
              </h3>
              <textarea 
                v-model="postData.summary"
                maxlength="180"
                placeholder="Nhập đoạn tóm tắt ngắn để hiển thị ngoài trang chủ..." 
                class="w-full h-40 border border-slate-200 rounded-xl p-3 text-sm text-slate-700 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all resize-none bg-slate-50 focus:bg-white"
              ></textarea>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6">
              <h3 class="font-bold text-slate-900 mb-4 uppercase tracking-widest text-[11px] flex items-center gap-2">
                <span class="material-symbols-outlined text-primary text-[18px]">tag</span> Tags bài viết
              </h3>
              <input 
                v-model="postData.tags" 
                placeholder="VD: Bandai, GoodSmile, Gundam (Cách nhau bằng dấu phẩy)" 
                class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all bg-slate-50"
              >
            </div>
          </div>
        </div>
...
<script setup>
  ...
  // Xử lý Upload Ảnh Thumbnail (Bên cột phải)
  ...
  const submitPost = async (status) => {
    ...
    try {
        ...
        const result = await response.json();
        if (response.ok) {
            toastStore.showToast("Xuất bản bài viết thành công!", "success");
