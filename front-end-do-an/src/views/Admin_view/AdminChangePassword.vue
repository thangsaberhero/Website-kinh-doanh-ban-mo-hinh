<template>
    <div class="bg-slate-100 min-h-screen font-body flex w-full text-slate-800 relative">
      
      <div v-show="isMobileMenuOpen" @click="isMobileMenuOpen = false" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-40 md:hidden transition-opacity"></div>
...
<script setup>
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/admin_info/change_password', {
        method: 'POST',
        headers: {
