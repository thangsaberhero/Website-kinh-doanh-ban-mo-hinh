            ...
            <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 w-full">
              ...
              <div class="flex flex-wrap items-center gap-2 w-full xl:w-auto">
                ...
                <button v-if="totalOrders > itemsPerPage"
                        ...
                        class="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 px-4 py-2.5 rounded-xl shadow-sm transition-all">
              ...
              <div class="flex items-center gap-3 shrink-0 self-end sm:self-auto">
                <button @click="isFilterPanelOpen = true" class="px-4 py-2 flex items-center justify-center gap-1.5 border border-slate-200 bg-white rounded-xl text-slate-500 hover:text-primary hover:border-primary transition-all shadow-sm font-bold text-xs" title="Bộ lọc nâng cao">
                ...
                <div class="relative">
                  <button 
                    ...
                    class="px-4 py-2 flex items-center justify-center gap-1.5 border border-slate-200 bg-white rounded-xl text-slate-500 hover:text-primary hover:border-primary transition-all shadow-sm font-bold text-xs"
          ...
          <div class="p-6 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-slate-100 bg-slate-50/30">
            ...
            <div class="flex items-center gap-3">
              <span class="text-xs font-bold text-slate-400">
                Hiển thị {{ startItem }} - {{ endItem }} của {{ totalOrders }} đơn
              </span>
            ...
            <div v-if="totalPages > 1" class="flex items-center gap-1.5">
              ...
              <template v-for="(p, index) in visiblePages" :key="index">
                ...
                <span v-else class="w-8 h-8 flex items-center justify-center text-slate-400 text-xs font-bold">
                  ...
                </span>
              </template>
          ...
          <div class="p-6 md:p-8 overflow-y-auto flex-1 custom-scrollbar bg-slate-50/50">
            <div class="space-y-6">
              ...
              <div class="bg-white border border-slate-200 rounded-2xl p-6 relative overflow-hidden shadow-sm">
                ...
                <div v-if="getCurrentStatusCode() == 5 || getCurrentStatusCode() == 6" class="flex items-center gap-3 bg-rose-50 border border-rose-100 p-4 rounded-xl relative z-10">
                  ...
                  <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-4 w-full">
                    ...
                    <div class="flex flex-col items-center text-center gap-2 flex-1">
                      ...
                      <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-medium font-mono mt-0.5 whitespace-nowrap">
                          {{ selectedOrder.LichSuTrangThai?.find(l => l.MaTrangThai == 1) ? new Date(selectedOrder.LichSuTrangThai.find(l => l.MaTrangThai == 1).Thoigian).toLocaleString('vi-VN', {hour: '2-digit', minute:'2-digit', day: '2-digit', month: '2-digit', year: 'numeric'}) : '--:--' }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center text-center gap-2 flex-1">
                      ...
                      <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-medium font-mono mt-0.5 whitespace-nowrap">
                          {{ selectedOrder.LichSuTrangThai?.find(l => l.MaTrangThai == 2) ? new Date(selectedOrder.LichSuTrangThai.find(l => l.MaTrangThai == 2).Thoigian).toLocaleString('vi-VN', {hour: '2-digit', minute:'2-digit', day: '2-digit', month: '2-digit', year: 'numeric'}) : '--:--' }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center text-center gap-2 flex-1">
                      ...
                      <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-medium font-mono mt-0.5 whitespace-nowrap">
                          {{ selectedOrder.LichSuTrangThai?.find(l => l.MaTrangThai == 3) ? new Date(selectedOrder.LichSuTrangThai.find(l => l.MaTrangThai == 3).Thoigian).toLocaleString('vi-VN', {hour: '2-digit', minute:'2-digit', day: '2-digit', month: '2-digit', year: 'numeric'}) : '--:--' }}
                        </p>
                      </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center text-center gap-2 flex-1">
                      ...
                      <div>
                        ...
                        <p class="text-[10px] text-slate-400 font-medium font-mono mt-0.5 whitespace-nowrap">
                          {{ selectedOrder.LichSuTrangThai?.find(l => l.MaTrangThai == 4) ? new Date(selectedOrder.LichSuTrangThai.find(l => l.MaTrangThai == 4).Thoigian).toLocaleString('vi-VN', {hour: '2-digit', minute:'2-digit', day: '2-digit', month: '2-digit', year: 'numeric'}) : '--:--' }}
                        </p>
                      </div>
                    </div>
                ...
                <div class="flex flex-col gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  ...
                  <div v-if="externalOrderForm.ThuTienNgay" class="flex flex-col gap-4 animate-[fadeIn_0.2s_ease-out] pt-3 border-t border-slate-200/60">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <label :class="externalOrderForm.PhuongThucTT === 5 ? 'bg-emerald-100 text-emerald-700 border-emerald-500 shadow-sm' : 'bg-white border-slate-200 text-slate-500 hover:border-emerald-300 hover:text-emerald-600 hover:bg-emerald-50/30'" class="border rounded-xl p-3 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 text-center">
                        <input type="radio" v-model="externalOrderForm.PhuongThucTT" :value="5" class="hidden">
                        <span class="material-symbols-outlined text-[24px]">payments</span>
                        <span class="text-xs font-bold uppercase tracking-widest mt-1">Tiền mặt</span>
                      </label>

                      <label :class="externalOrderForm.PhuongThucTT === 4 ? 'bg-blue-100 text-blue-700 border-blue-500 shadow-sm' : 'bg-white border-slate-200 text-slate-500 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50/30'" class="border rounded-xl p-3 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 text-center">
                        ...
                        <span class="text-xs font-bold uppercase tracking-widest mt-1">Chuyển khoản</span>
                      </label>
                    </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                <label :class="collectionMethod === 5 ? 'border-emerald-500 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-slate-200 bg-white text-slate-500 hover:border-emerald-300 hover:text-emerald-600 hover:bg-emerald-50/30'" class="border rounded-xl p-3 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 text-center">
                  <input type="radio" v-model="collectionMethod" :value="5" class="hidden">
                  <span class="material-symbols-outlined text-[22px]">payments</span>
                  <span class="text-[10px] font-bold uppercase tracking-wider mt-0.5">Tiền mặt</span>
                </label>
                
                <label :class="collectionMethod === 4 ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-sm' : 'border-slate-200 bg-white text-slate-500 hover:border-blue-300 hover:text-blue-600 hover:bg-blue-50/30'" class="border rounded-xl p-3 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 text-center">
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-wider mt-0.5">Chuyển khoản</span>
                </label>

                <label v-if="alreadyPaidAmount > 0" :class="collectionMethod === 3 ? 'border-orange-500 bg-orange-50 text-orange-700 shadow-sm' : 'border-slate-200 bg-white text-slate-500 hover:border-orange-300 hover:text-orange-600 hover:bg-orange-50/30'" class="border rounded-xl p-3 cursor-pointer transition-all flex flex-col items-center justify-center gap-1 text-center sm:col-span-1 col-span-2">
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-wider mt-0.5">Thu hộ COD</span>
                </label>
...
const exportExcelReport = async () => {
    ...
    try {
      ...
      a.download = `Bao_Cao_Don_Hang_FigureCollect_${dateStr}.xlsx`;
