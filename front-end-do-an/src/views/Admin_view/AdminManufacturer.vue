          ...
          <div class="p-6 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-slate-100 bg-slate-50/30">
            ...
            <div class="flex items-center gap-3">
              <span class="text-xs font-bold text-slate-400">
                Hiển thị {{ startItem }} - {{ endItem }} của {{ totalBrands }} hãng sản xuất
              </span>
...
const changeItemsPerPage = () => {
  currentPage.value = 1;
  fetchManufacturers(); // Gọi lại hàm tải danh sách của bồ
};
        ...
        manufacturers.value = result.data.map(item => {
          ...
          if (item.Logo) {
             if (item.Logo.startsWith('http')) {
                ...
                imageUrl = `${API_BASE_URL}/Images_brand/${item.Logo}`;
             }
          }
