      ...
      <div ref="chatBox" class="h-80 overflow-y-auto p-4 flex flex-col gap-3 scroll-smooth">
        ...
        >
          ...
          <div v-if="msg.products && msg.products.length > 0" class="mt-2 flex flex-col gap-2">
            ...
            >
              <img :src="getProductImage(sp.LinkAnh)" class="w-14 h-14 object-cover rounded-lg bg-white" alt="Product image" />
...
const getProductImage = (linkAnh) => {
  if (!linkAnh) return '/Images_product/default.jpg';
  if (/^https?:\/\//i.test(linkAnh)) return linkAnh;
  return `/Images_product/${linkAnh}`;
};
