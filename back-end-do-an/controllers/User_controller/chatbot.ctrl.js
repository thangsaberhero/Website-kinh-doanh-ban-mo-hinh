...
const cleanProductKeyword = (rawText = "") => {
    ...
    const stopWords = [
        ...
        "có", "co", "còn", "con", "không", "khong", "ko", "nào", "nao", "và", "va", "với", "voi", "về", "ve", "các", "cac",
        "nhé", "nhe", "nha", "nè", "ne", "đi", "di", "thế", "the", "vậy", "vay", "hả", "ha", "đang", "dang","thì","thi","vài", "vai",
...
const productSelect = `
    ...
    m.Series,
    m.AnhDaiDien,
    COALESCE(NULLIF(m.AnhDaiDien, ''), (SELECT LinkAnh FROM AnhMoHinh WHERE MaMoHinh = m.MaMoHinh LIMIT 1)) AS LinkAnh,
    ...
    try {
        ...
        switch (intentName) {
            ...
            case "kiem_tra_ton_kho": {
                const keyword = cleanProductKeyword(dfResult.keywords || cleanMessage);
                if (!keyword) {
            ...
            case "tim_theo_danh_muc": {
                const danhMuc = decision.categoryKeyword || detectCategoryKeyword(dfResult.DanhMuc || dfResult.keywords || cleanMessage) || cleanProductKeyword(dfResult.DanhMuc || cleanMessage);
                if (!danhMuc) {
            ...
            case "tu_van_theo_gia": {
                const { minPrice, maxPrice, keyword, searchMode } = parsePriceAndKeyword(cleanMessage, dfResult.keywords);
                ...
                const priceText = maxPrice >= 999999999
                    ? `trên ${Math.round(minPrice).toLocaleString("vi-VN")}đ`
                    : minPrice <= 0
                        ? `dưới ${Math.round(maxPrice).toLocaleString("vi-VN")}đ`
                        : `từ ${Math.round(minPrice).toLocaleString("vi-VN")}đ đến ${Math.round(maxPrice).toLocaleString("vi-VN")}đ`;
            ...
            case "tra_cuu_don_hang": {
                const { sdt, maDon } = extractOrderLookup(cleanMessage, dfResult);
                if (!sdt && !maDon) {
