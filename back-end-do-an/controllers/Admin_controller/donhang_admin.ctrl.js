...
const donhang_admin = {
            ...
            products.forEach((item, index) => {
                ...
                inputRows += `
                    <tr>
                        ...
                        <td style="text-align: right; border-bottom: 1px dashed #ccc; padding: 6px 0;">${formatMoney(thanhTienBan)}</td>
                    </tr>
                `;
            });
    ...
    in_hoa_don_hang_loat: async (req, res) => {
        ...
        try {
            ...
            // 1. ĐỊNH DẠNG CHUNG (Tiền tệ, Ngày tháng & Logo)
            const formatMoney = (amount) => Number(amount).toLocaleString('vi-VN') + ' đ';
            const formatDate = (dateStr) => {
                const d = new Date(dateStr);
                return `${d.getDate().toString().padStart(2,'0')}/${(d.getMonth()+1).toString().padStart(2,'0')}/${d.getFullYear()} ${d.getHours().toString().padStart(2,'0')}:${d.getMinutes().toString().padStart(2,'0')}`;
            ...
            <head>
                ...
                <style>
                    body { font-family: 'Arial', sans-serif; font-size: 13px; color: #000; margin: 0; padding: 20px; line-height: 1.4; background: #eee; }
                    
                    /* QUAN TRỌNG: Ép ngắt trang sau mỗi hóa đơn, giao diện tờ A5/K80 */
                    .invoice-page { 
                        max-width: 600px; 
                    ...
                    .data-table { width: 100%; border-collapse: collapse; margin-top: 10px; }
                    .data-table th { border-bottom: 2px solid #000; padding: 6px 0; font-weight: bold; font-size: 12px; text-align: left;}
                    .data-table td { border-bottom: 1px dashed #ccc; padding: 6px 0; }
                    ...
                    /* Tự kích hoạt ẩn các thành phần thừa và xóa màu nền khi in thật */
                    @media print {
                        body { padding: 0; margin: 0; background: transparent; }
                        @page { size: auto; margin: 5mm; }
                        .invoice-page { margin: 0; max-width: 100%; box-shadow: none; padding: 10px; }
                    }
                </style>
            </head>
            <body>
            ...
            // 3. LẶP QUA TỪNG ĐƠN HÀNG ĐỂ LẤY DỮ LIỆU & RÁP GIAO DIỆN
            for (let i = 0; i < orderIds.length; i++) {
                ...
                if (donhang_info.length === 0) continue;
                const dh_data = donhang_info[0];
                ...
                // Xử lý tính toán cho hóa đơn này
                let inputRows = '';
                let tongTienGiamCacMon = 0;
                ...
                products.forEach((item, index) => {
                    ...
                    inputRows += `
                        <tr>
                            ...
                            <td style="text-align: right;">${formatMoney(thanhTienBan)}</td>
                        </tr>
                    `;
                });

                const tongGiamVoucher = (dh_data.TongTien - tongTienGiamCacMon) - dh_data.ThanhTien;
                const tongGiamGiaThucTe = Math.max(0, tongTienGiamCacMon + (tongGiamVoucher > 0 ? tongGiamVoucher : 0));

                // Ráp vào khung của một trang (Một tờ hóa đơn)
                combinedHtml += `
                    <div class="invoice-page">
                        <div class="header">
                            ...
                            <div class="divider"></div>
                            <h3 style="margin: 10px 0; font-size: 18px;">HÓA ĐƠN BÁN HÀNG</h3>
                            <p style="font-style: italic;">Mã tra cứu: ${dh_data.MaDonHangHienThi}</p>
                        </div>

                        <table class="info-table">
                            <tr>
                                <td style="width: 40%"><strong>Ngày lập đơn:</strong> ${formatDate(dh_data.NgayLapDon)}</td>
                                <td style="width: 60%"><strong>Khách hàng:</strong> ${dh_data.TenNguoiNhan}</td>
                            </tr>
                            <tr>
                                <td><strong>Trạng thái:</strong> ${dh_data.TrangThaiThanhToan}</td>
                                <td><strong>Số điện thoại:</strong> ${dh_data.SDTNguoiNhan}</td>
                            </tr>
                            <tr>
                                <td colspan="2"><strong>Địa chỉ giao:</strong> ${dh_data.DiaChiGiao || 'Nhận tại quầy'}</td>
                            </tr>
                            ${dh_data.Note ? `<tr><td colspan="2"><strong style="color:#ff3d00">Ghi chú đơn:</strong> ${dh_data.Note}</td></tr>` : ''}
                        </table>
                        ...
                        <table class="summary-table">
                            <tr>
                                <td style="text-align: right; width: 70%;"><strong>Tổng tiền hàng:</strong></td>
                                <td style="text-align: right; width: 30%; font-weight: bold;">${formatMoney(dh_data.TongTien)}</td>
                            </tr>
                            ${tongGiamGiaThucTe > 0 ? `
                            <tr>
                                <td style="text-align: right; color: red;"><strong>Tổng cộng giảm giá:</strong></td>
                            ...
                            <tr>
                                <td style="text-align: right; font-size: 11px; color: #666;">Mã Voucher áp dụng:</td>
                                <td style="text-align: right; font-size: 11px; color: #666; font-weight:bold;">${dh_data.MaVoucher}</td>
                            </tr>` : ''}
                            <tr>
                                <td style="text-align: right; font-size: 15px; border-top: 1px solid #000; padding-top: 8px;"><strong>TỔNG THANH TOÁN:</strong></td>
                                <td style="text-align: right; font-size: 16px; font-weight: bold; color: #ff3d00; border-top: 1px solid #000; padding-top: 8px;">${formatMoney(dh_data.ThanhTien)}</td>
                            </tr>
                        </table>
    ...
    xuatExcelDonHang: async (req, res) => {
        try {
            ...
            const sql_nhatky = `
                SELECT cttt.MaDH, dh.MaDonHangHienThi, tt.TenTrangThai, cttt.Thoigian
                FROM ChiTietTrangThai cttt
                INNER JOIN TrangThai tt ON cttt.MaTrangThai = tt.MaTrangThai
                INNER JOIN DonHang dh ON cttt.MaDH = dh.MaDH
                ${whereClause}
            ...
            const taoHeaderBaoCao = (ws, titleName, endCol, filterText) => {
            ...
            let rawFilterText = (NgayBatDau && NgayKetThuc) ? `Từ ${formatDateVN(NgayBatDau)} đến ${formatDateVN(NgayKetThuc)}` : 'Tất cả thời gian';
            if (trangthaitt) rawFilterText += ` | TT: ${trangthaitt}`;
            if (minPrice || maxPrice) {
                const min = minPrice ? new Intl.NumberFormat('vi-VN').format(minPrice) : '0';
                const max = maxPrice ? new Intl.NumberFormat('vi-VN').format(maxPrice) : 'Max';
                rawFilterText += ` | Đơn: ${min}đ - ${max}đ`;
            }
            ...
            ws1.columns = [
                ...
                { key: 'ChiTietSanPham', width: 45 },
                { key: 'ThanhTien', width: 18 },
            ];

            taoHeaderBaoCao(ws1, 'BÁO CÁO DOANH THU BÁN HÀNG', 'F', rawFilterText);

            const headerRow1 = ws1.getRow(9);
            headerRow1.values = ['STT', 'Mã đơn hàng', 'Khách hàng', 'Ngày mua', 'Chi tiết sản phẩm', 'Tổng tiền (VNĐ)'];
            ...
            donHangs.forEach((item, index) => {
                ...
                row.eachCell((cell, colNum) => {
                    ...
                    cell.border = blackBorder; 
                    if (colNum === 6) { // Cột tiền
                        cell.alignment = { horizontal: 'right', vertical: 'middle' };
                        cell.numFmt = '#,##0';
                    } else if (colNum === 1 || colNum === 2 || colNum === 4) {
            ...
            taoHeaderBaoCao(ws2, 'NHẬT KÝ CHUYỂN TRẠNG THÁI ĐƠN HÀNG', 'D', rawFilterText);

            const headerRow2 = ws2.getRow(9);
            headerRow2.values = ['STT', 'Mã đơn hàng', 'Trạng thái cập nhật', 'Thời gian ghi nhận'];
            headerRow2.height = 25;
            headerRow2.eachCell((cell) => {
                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF222532' } }; 
                cell.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 11 };
                cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
                cell.border = blackBorder;
            });

            nhatKys.forEach((item, index) => {
    ...
    xuat_excel_giao_dich: async (req, res) => {
        try {
            ...
            ws.mergeCells(`A5:${endCol}5`);
            const titleCell = ws.getCell('A5');
            titleCell.value = 'BÁO CÁO GIAO DỊCH THANH TOÁN';
            titleCell.font = { size: 18, bold: true, color: { argb: 'FF222532' }, name: 'Space Grotesk' };
            titleCell.alignment = { horizontal: 'center', vertical: 'middle' }; 
            ...
            headerRow.values = ['STT', 'Thời gian', 'Mã đơn hàng', 'Khách hàng', 'Phương thức', 'Mã đối soát (Bank/Ví)', 'Nghiệp vụ', 'Số tiền (VNĐ)'];
            headerRow.height = 25;
            ...
            transactions.forEach((tx, index) => {
                ...
                row.values = [
                    index + 1,
                    new Date(tx.NgayThanhToan).toLocaleString('vi-VN'),
                    tx.MaDonHangHienThi,
                    tx.TenNguoiNhan || 'Khách vãng lai',
                    tx.TenPhuongThuc,
                ...
                row.eachCell((cell, colNumber) => {
                    ...
                    if (colNumber === 1 || colNumber === 2 || colNumber === 3 || colNumber === 5) {
                        ...
                        if (tx.SoTienGiaoDich > 0) {
                            cell.font = { size: 10, name: 'Manrope', color: { argb: 'FF10B981' }, bold: true }; 
                        } else {
                            cell.font = { size: 10, name: 'Manrope', color: { argb: 'FFE11D48' }, bold: true }; 
                        }
                    } else {
                        cell.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
                    }
